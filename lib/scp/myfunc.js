...
m.quoted.fromMe = m.quoted.sender === (sho.user && sho.user.id)
m.quoted.text = m.quoted.text || m.quoted.caption || m.quoted.conversation || m.quoted.contentText || m.quoted.selectedDisplayText || m.quoted.title || ''
m.quoted.mentionedJid = m.msg.contextInfo ? m.msg.contextInfo.mentionedJid : []
m.getQuotedObj = m.getQuotedMessage = async () => {
if (!m.quoted.id) return false
let q = await store.loadMessage(m.chat, m.quoted.id, sho)
return exports.smsg(sho, q, store)
}
 ...
 * @returns 
 */
m.quoted.delete = () => sho.sendMessage(m.quoted.chat, {
delete: vM.key
})
 ...
 */
m.reply = (text, chatId = m.chat, options = {}) => Buffer.isBuffer(text) ? sho.sendMedia(chatId, text, 'file', '', m, {
...options
}) : sho.sendText(chatId, text, m, {
...options
})
/**
 * Copy this message
 */
m.copy = () => exports.smsg(sho, M.fromObject(M.toObject(m)))
...
sho.appenTextMessage = async(text, chatUpdate) => {
let messages = await generateWAMessage(m.chat, { text: text, mentions: m.mentionedJid }, {
userJid: sho.user.id,
quoted: m.quoted && m.quoted.fakeObj
})
messages.key.fromMe = areJidsSameUser(m.sender, sho.user.id)
...
sho.ev.emit('messages.upsert', msg)
}
